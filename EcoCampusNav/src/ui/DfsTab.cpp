...
DfsTab::DfsTab(NavigationService& nav, ScenarioManager& scenario, CampusMapView* mapView,
    ...
    : QWidget(parent), nav_(nav), scenario_(scenario), mapView_(mapView), graph_(graph)
{
    ...
    btn_run_ = new QPushButton("Ejecutar DFS", this);
    ...
    connect(btn_run_, &QPushButton::clicked, this, &DfsTab::onRunDfs);
}

void DfsTab::refreshNodeList() {
    combo_start_->clear();
...
void DfsTab::onRunDfs() {
    QString startId = combo_start_->currentData().toString();
    if (startId.isEmpty()) return;

    auto result = nav_.runDfs(startId.toStdString(), scenario_.isMobilityReduced());
